    ...
    override fun updateState(
        ...
        return when (val result = transition(expected, next)) {
            ...
            is StateTransitionResult.Allowed -> {
                events[eventId] = current.withState(result.state, metadata ?: current.metadata)
                RepositoryResult.Applied
            }
        }
    }
...
class InMemoryEventPipeline(
    ...
    override fun process(eventId: EventId, expiration: EventExpirationPolicy): PipelineResult {
        ...
        return synchronized(lock) {
            ...
            when (event.state) {
                EventState.INITIATED -> advance(event, EventState.COLLECTING, PipelineStage.CONFIRMED)
                EventState.COLLECTING -> runEvidence(event)
                EventState.READY -> runDelivery(event)
                ...
                EventState.DEFERRED -> resume(event)
    ...
    private fun resume(event: SecurityEvent): PipelineResult = when {
        event.metadata["deferredStage"] == PipelineStage.DELIVERY.name -> runDelivery(event)
        else -> runEvidence(event)
    }

    private fun runEvidence(event: SecurityEvent): PipelineResult {
        return when (val outcome = evidence.execute(event)) {
            StageOutcome.Completed -> {
                val updated = transitionAndRead(event, EventState.READY)
    ...
    private fun runDelivery(event: SecurityEvent): PipelineResult {
        val delivering = transitionAndRead(event, EventState.DELIVERING)
        ...
        return when (val outcome = delivery.execute(delivering)) {
            StageOutcome.Completed -> {
                val completed = transitionAndRead(delivering, EventState.DELIVERED)
    ...
    private fun advance(event: SecurityEvent, next: EventState, stage: PipelineStage): PipelineResult {
        val updated = transitionAndRead(event, next)
    ...
    private fun transitionAndRead(event: SecurityEvent, next: EventState): SecurityEvent? {
        val result = repository.updateState(event.id, event.state, next)
    ...
    private fun fail(event: SecurityEvent, failure: PipelineFailure): PipelineResult {
        repository.updateState(event.id, event.state, EventState.FAILED_FINAL)
        return PipelineResult.Failed(repository.find(event.id) ?: event, failure)
    }

    private fun expire(event: SecurityEvent): PipelineResult {
        val result = repository.updateState(event.id, event.state, EventState.EXPIRED)
        return if (result is RepositoryResult.Applied) {
