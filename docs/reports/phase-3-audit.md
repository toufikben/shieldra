...
The workflow now runs the existing Group A diagnostics, Group B/Group C build, Group C boundary scan, and the new `phase3-review-check.sh` step. Remote CI verification is still required after the milestone is pushed. No runtime emulator/device is available in this sandbox, so screenshot-level visual verification, large-font runtime verification, and Arabic runtime layout verification remain **UNKNOWN**, not claimed as passed.
