# Authority Index

| Authority | Version | Sections | Status | Source | Last Updated |
|---|---|---:|---|---|---|
| Product Freeze | v5 | Not supplied | BLOCKED | Not provided in the task attachments or current repository; reviewed in `docs/reports/product-freeze-v5-review.md` | 2026-09-22 |
| Phase 1 Specification | Sections 1–43 | Not supplied in full | BLOCKED | Not provided in the task attachments or current repository; reviewed in `docs/reports/product-freeze-v5-review.md` | 2026-09-22 |
| Group B supplied execution scope | Current attachment | 1–27 | AVAILABLE PARTIAL | `phase-1-group-b-supplied-scope.md` | 2026-09-20 |
| Group C supplied execution scope | Current attachment | 1–29 | AVAILABLE PARTIAL | `phase-1-group-c-supplied-scope.md` | 2026-09-21 |
| External Reviewer decisions | Not specified | N/A | NONE PROVIDED | No decision document supplied | 2026-09-21 |
| Master audit and controlled implementation protocol | User-supplied attachment | 38 sections | AVAILABLE PROCESS AUTHORITY | `shieldra-master-audit-controlled-protocol.md` | 2026-09-22 |
| SHIELDRA internal decisions | Unified approved prompts | G0–G7 | AVAILABLE INTERNAL AUTHORITY | `../../SHIELDRA_DECISIONS.md` | 2026-09-22 |
| Phase 4 decision register | Unified approved prompts | G0–G7 | AVAILABLE INTERNAL REGISTER | `../PHASE4_DECISION_REGISTER.md` | 2026-09-22 |
| Batch 4a Android capability research | Official Android Developers sources | Sensors, telephony, background, notifications, camera, location | AVAILABLE TECHNICAL RESEARCH — NO IMPLEMENTATION AUTHORITY | `../reports/batch-4a-android-capability-research.md` | 2026-09-22 |
| Batch 4a device/API test matrix | Internal execution plan | API 26–35 and physical OEM/telephony profiles | AVAILABLE TEST PLAN — EXECUTION PENDING | `../PHASE4_DEVICE_TEST_MATRIX.md` | 2026-09-22 |
| Batch 4b device-execution preflight | Environment audit | ADB, emulator, AVD, and physical-device availability | BLOCKED — NO DEVICE ENVIRONMENT | `../reports/batch-4b-device-execution-preflight.md` | 2026-09-22 |
| Batch 4c storage and Keystore candidate | Local implementation and verification | Versioned schema, explicit encryption policy, Android Keystore candidate | CANDIDATE — G4 STILL OPEN | `../reports/batch-4c-storage-keystore-schema.md` | 2026-09-23 |
| G4 security acceptance gap review | Repository security review | 18-point closure checklist and required evidence | AVAILABLE REVIEW — G4 NOT READY FOR CLOSURE | `../reports/g4-security-acceptance-gap-review.md` | 2026-09-23 |
| G4-A conservative decision set | Supplied G4 Security Storage Decision Freeze | Data inventory, schema/migration/lifecycle/backup/crypto/key/auth/corruption proposals, threat model, Android checklist, acceptance checklist | AVAILABLE PROPOSAL — REQUIRES APPROVAL | `../security/g4-a-decision-set.md` and adjacent G4-A documents | 2026-09-23 |

Product Freeze v5 remains the highest authority. This repository does not claim that the supplied Group B or Group C scopes are complete replacements for Product Freeze v5 or Phase 1 Sections 1–43. Any unresolved requirement is recorded as `NEEDS REVIEW` or `BLOCKED`. See [`../reports/product-freeze-v5-review.md`](../reports/product-freeze-v5-review.md) for the current gap review.
